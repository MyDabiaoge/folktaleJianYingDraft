...
from LLM.jimeng_reverse import create_image_save_file
...
# 对create_image_save_file函数应用重试装饰器
retryable_create_image_save_file = retry_on_exception(**IMAGE_GEN_RETRY_CONFIG)(create_image_save_file)
...
def session_worker(session_id, scenes_queue, story_name, db_lock, failed_scenes_queue=None):
    ...
    while True:
        ...
        try:
            thread_safe_print(f"线程{session_id}处理场景 {scene.index}，提示词: {scene.prompt[:50]}...")

            # 使用带重试的函数调用
            image_paths = retryable_create_image_save_file(
                session_id, scene.index, story_name, scene.prompt
            )

            # 使用数据库锁确保数据库操作线程安全
            with db_lock:
                scene.image_path1 = image_paths[0] if len(image_paths) > 0 else None
                scene.image_path2 = image_paths[1] if len(image_paths) > 1 else None
                scene.image_path3 = image_paths[2] if len(image_paths) > 2 else None
                scene.image_path4 = image_paths[3] if len(image_paths) > 3 else None
                scene.save()
...
if __name__ == '__main__':
    ...
    str_one = input("默认只补全没有图片的scene,若要重头到尾重新执行请输入1:")
    is_rerun = False
    if str_one == "1":
        is_rerun = True
    create_images(int(story_id), is_rerun)
